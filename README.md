...
[项目简介](#-项目简介) / [适用场景](#-适用场景) / [支持平台](#-支持平台) / [安装说明](#️-安装说明) / [配置说明](#️-配置说明) / [插件命令](#️-插件命令) / [工作流程说明](#-工作流程说明) / [排障说明](#-排障说明)

</div>

## 📖 项目简介

这是一个给 AstrBot 使用的媒体桥接插件。

作用只有一件事。
把聊天中的分享链接下载成文件。
再把文件转换成稳定的 HTTP 地址。
最后通过 NapCat 原生消息发送出去。
...
### ℹ️ 关于 NapCat 本地文件转 URL

通常不需要额外开启 NapCat 的 本地文件到 URL 之类开关。

这个插件的发送逻辑不是把本地路径直接丢给 NapCat。
而是先把文件放进静态目录。
再生成可访问的 HTTP 地址。
最后把这个 HTTP 地址作为 `video/file` 发出去。
